...
with container:
    ...
    if submit_button and user_input:
        if uploaded_files:
            # check if user specified a file name in their query
            file_name_specified_in_query = False
            for uploaded_file in uploaded_files:
                if uploaded_file.name.replace(' ', '_') in user_input.replace(' ', '_'):
                    file_name_specified_in_query = True
                    break

            # use the specified file to answer the question
            if file_name_specified_in_query:
                for uploaded_file in uploaded_files:
                    if uploaded_file.name.replace(' ', '_') in user_input.replace(' ', '_'):
                        if uploaded_file.type == "text/csv":
                            agent = st.session_state['agents'][uploaded_file.name]
                            output = agent.run(user_input)
                        elif uploaded_file.type == "application/pdf":
                            faiss_index = st.session_state['faiss_indices'][uploaded_file.name]
                        ...
                        break
            # if no file name was specified in the query, use the last uploaded file to answer the question
            else:
